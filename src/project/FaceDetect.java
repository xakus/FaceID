...
//                                int                y     = rect.y();
        ...
        public void faceRectangleAndText(opencv_core.IplImage img, opencv_core.CvSeq cvSeq,List<opencv_core.IplImage> m){
                ...
                if (total > 0){
                        ...
                        for (int i = 0; i < total; i++){
                                ...
                                //                x=x+((int)(h/1.618)/2)/2;
                                //                w= w-
                                opencv_core.IplImage image  = m.get(i);//operation.faceCut(img, x, y, w, h);
        ...
        public void faceRectangleAndText(opencv_core.IplImage img, opencv_core.CvSeq cvSeq){
                ...
                if (total > 0){
                        ...
                        for (int i = 0; i < total; i++){
                                ...
                                opencv_core.IplImage image1 = operation.resizeImage(image);
                                //Eye
                                List<opencv_core.Rect> rects =eyeDetect.eyeDetect2(image1);
                                 EyeCenterAndAngle     angle =Operation.angle(rects);
                                 System.out.println(angle.getAngle()+"");
                                image1 = Operation.rotate(image1,angle.getCenterPoint(),angle.getAngle());
                                ///
                                FacePredict          pr     = predict.facePredict(image1);
                                if (pr.getConfidence().get() < Const.DETECT_LIMIT || true){
                                        text = "N=" + Const.getName.get(pr.getLabel().get()) + " P=" + pr.getConfidence().get() + "%";
